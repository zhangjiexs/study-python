...
class Spider():
    ...
    def makeDir(self, path):
        ...
        if not E:
            ...
            #更改环境变量用os.chdir()
            os.makedirs(os.path.join('D:\Photos\TieBa', self.path))
    ...
    def saveImage(self, detailURL, name):
        try:
            ...
            return None
        fileName = name + '.' + 'jpg'
        f=open(r'D:\Photos\TieBa\%s\%s'%(self.path, fileName), 'wb')
        f.write(data)
        f.close()
...
Now = "https://tieba.baidu.com/f?kw=%E6%9D%8E%E6%AF%85"
spider = Spider()
spider.getAllPage(Now)